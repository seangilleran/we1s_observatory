...
from we1s_observatory.libs.zipeditor import zip_scanner
        ...
        try:
            with zipfile.ZipFile(zip_file, "r") as zfile:
                ...
                zip_output_dir = os.path.join(
                    output_dir, os.path.basename(zip_file).rsplit(".zip")[0]
                )
                os.makedirs(zip_output_dir, exist_ok=True)

                # duplicates
                zfile_dupes = io.TextIOWrapper(
                    zfile.open("_duplicates.txt", "r")
                ).readlines()
                ...
                # deletes
                zfile_deletes = io.TextIOWrapper(
                    zfile.open("_deletes.txt", "r")
                ).readlines()
...
if __name__ == "__main__":
    ...
    PARSER.add_argument(
        "-i", "--inpath", default=".", help="input path for directory of zips"
    )
    PARSER.add_argument(
        "-o",
        "--outpath",
        default="dupe_inspect",
