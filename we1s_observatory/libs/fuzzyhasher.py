...
class FuzzyHasher:
    ...
    def compare_files(self, paths):
        ...
        for chunksize, hash_list in hash_by_file.items():
            ...
            chunk_pairs = itertools.combinations(
                [hash_entry for hash_entry in hash_list.items()], 2
            )
    ...
    def compare_files_in_dir(self, source_path="data"):
        ...
        files = [
            entry.path for entry in os.scandir(source_path) if entry.path.endswith(".json")
        ]
        errors = []
        for file in files:
            with open(file, "r") as jsonfile:
                try:
                    hash_string = json.load(jsonfile)["content-hash-ssdeep"]
                except (
                    json.decoder.JSONDecodeError,
                    KeyError,
                    PermissionError,
