...
from we1s_observatory.libs.deduper import LinkFilter
from we1s_observatory.libs.fuzzyhasher import FuzzyHasher
from we1s_observatory.libs.preprocess import Preprocessor, content_field_standardize
from we1s_observatory.libs.zipeditor import ZipEditor, zip_scanner


def zip_batch_process(
    zip_dir_root="",
    ...
    for zip_file in zip_files:
        ...
        with ZipEditor(zip_file) as zed:
            ...
            try:
                ...
                result_list = [
                    [str(item).replace(zed.getdir() + "/", "") for item in row]
                    for row in results
                ]
                if result_list:
                    ...
                    changed = True
                    with open(
                        os.path.join(zed.getdir(), "_duplicates.txt"), "w"
                    ) as dupefile:
                    ...
                    with open(
                        os.path.join(zed.getdir(), "_deletes.txt"), "w"
                    ) as delfile:
            ...
            pp.wikifier_output_dir = os.path.join(
                wikifier_output_dir, os.path.basename(zed.file).rsplit(".zip")[0]
            )
...
def main(args):
    ...
    zip_batch_process(
        zip_dir_root=args.inpath,
        source_field=args.content,
        preprocessing_log=args.log,
        wikifier_output_dir=args.wiki,
        skip_rerun=args.skip,
    )
...
if __name__ == "__main__":
    ...
    PARSER.add_argument(
        "-l",
        "--log",
        default="_preprocessing_log.csv",
